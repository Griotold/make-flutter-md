...
4. **표준 기능 (모든 앱 공통)** — 이 레포가 정의하는 신규 앱 필수 기능. v1에는 다음 1개:
   - **인앱 튜토리얼** — 3~4장 카드 캐러셀 (좌우 스와이프), 각 카드에 짧은 무성 영상 (3~5초, autoplay+loop) + 설명 텍스트
   - 영상은 빌드 타임 생성: Nano Banana로 이미지 생성 → image-to-video로 짧은 모션 클립, 카드별 독립 파일 `assets/tutorial/{lang}/card_N.mp4`
   - 텍스트는 영상이 아니라 Flutter UI 레이어 (다국어 리소스)
...
- **UX 패턴 (Sudoku 힌트 스타일):**
  ...
  - 각 카드 = 짧은 영상 (3~5초, 자동재생 + loop) + 설명 텍스트 1~2줄
  - 페이지 인디케이터 (점) 하단 표시, 첫 카드부터 마지막까지 자유 이동
  - 마지막 카드에 "시작하기" 버튼
- **포맷 원칙:**
  - **음성 없음.** 무성 영상 + 화면 위 텍스트로 충분 (음소거/공공장소 무관)
  - **이미지는 앱 UI를 정확히 표현하는 일러스트.** 가짜 다른 앱 화면 X. Nano Banana가 생성한 이미지는 실제 앱의 레이아웃/색상/요소를 반영해야 함.
- **생성 파이프라인 (빌드 타임, 런타임 생성 금지):**
  1. 각 카드별로 프롬프트 작성 (예: "스도쿠 보드 일러스트, 빈 셀 9x9, 가운데 셀에 빛나는 하이라이트, 상단에 'Tap any empty cell' 텍스트 공간, 앱 테마 컬러 #...")
  2. **Nano Banana** (Gemini 2.5 Pro image generation) 호출 → 카드별 이미지 1장씩 생성
  3. 그 이미지를 **시작 프레임으로 image-to-video 모델 호출** (Veo 또는 동급) → 3~5초 짧은 모션 클립 추출. 모션 프롬프트는 "탭 인디케이터가 살짝 움직임", "셀이 하이라이트되며 들어옴" 같은 작은 변화 수준.
  4. 출력: `assets/tutorial/{lang}/card_{1..4}.mp4` (카드별 독립 파일, 스티치 없음)
  5. 전체 빌드 스크립트: `dart run tool/build_tutorial.dart` (Gemini API 키는 `.env`에서 읽고 절대 커밋 금지)
- **재생 위젯:** Flutter `PageView` + 각 페이지에 `video_player` (autoplay, looping, 컨트롤 숨김). chewie 불필요. 카드 전환 시 보이지 않는 영상은 일시정지.
- **언어:** 디바이스 로케일에 따라 `ko/` 또는 `en/` 폴더 선택. 영어 없으면 한국어 폴백. **텍스트는 영상이 아니라 Flutter UI 레이어에 렌더** — 따라서 이미지/영상은 언어 무관, 텍스트만 다국어 리소스에서 가져옴 → 빌드 비용 절감(언어당 영상 재생성 안 함).
- **하드 룰:**
  - 스킵 가능 (Skip 버튼 항상 노출).
